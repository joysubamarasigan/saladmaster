	...
	<div class="select_wrapper <?php echo ( $is_multiple ) ? 'multiple' : '' ?>">
		...
		<select id="<?php echo $var[0].'-'.$var[2]; ?>" name="shortcode-<?php echo $var[0]; ?>" <?php echo $multiple ?> >
			...
			<?php foreach ( $var[1]['options'] as $key => $value) : ?>
				<option <?php ( $is_multiple ) ? selected( true, in_array( $key, (is_serialized( $var[1]['std'] )) ? unserialize( $var[1]['std'] ) : explode( ',', $var[1]['std'] ) ) ) : selected( $key, $var[1]['std'] ) ?> value="<?php echo $key; ?>"><?php echo $value; ?></option>
			<?php endforeach; ?>
		</select>
	</div>
